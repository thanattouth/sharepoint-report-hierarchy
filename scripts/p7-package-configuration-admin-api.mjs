...
const service = join(root, "services/configuration-admin-api");
const outputDirectory = join(root, "outputs");
const archive = join(outputDirectory, "configuration-admin-api.zip");
const staging = mkdtempSync(join(tmpdir(), "sp-config-admin-api-package-"));
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    status: "packaged",
    archive: "outputs/configuration-admin-api.zip",
