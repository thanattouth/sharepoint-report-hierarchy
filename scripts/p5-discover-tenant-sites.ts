...
const response = await fetch(`https://${hostname}/api/scanner/discover-sites`, {
  method: "POST",
  headers: { "x-functions-key": functionKey },
  redirect: "manual",
});
if (response.status !== 200) throw new Error(`Tenant Site discovery returned HTTP ${response.status}`);
...
for (const name of [
  ...
  "libraryPageCount",
  "sitesWithLibraries",
  "sitesWithoutLibraries",
  "failedSiteCount",
] as const) {
  if (!Number.isInteger(result[name]) || Number(result[name]) < 0) {
    throw new Error("Tenant Site discovery response is invalid");
  }
}
process.stdout.write(`${JSON.stringify({
  ...
  libraryPageCount: result.libraryPageCount,
  sitesWithLibraries: result.sitesWithLibraries,
  sitesWithoutLibraries: result.sitesWithoutLibraries,
  failedSiteCount: result.failedSiteCount,
