...
const service = join(root, "services/report-cache-api");
const outputDirectory = join(root, "outputs");
const archive = join(outputDirectory, "report-cache-api.zip");
const staging = mkdtempSync(join(tmpdir(), "sp-report-api-package-"));
...
try {
  ...
  process.stdout.write(`${JSON.stringify({
    status: "packaged",
    archive: "outputs/report-cache-api.zip",
