...
const response = await fetch(`https://${hostname}/api/scanner/review-wave-1`, {
  method: "POST",
  headers: { "x-functions-key": functionKey },
  redirect: "manual",
});
if (response.status !== 200) throw new Error(`Wave 1 review returned HTTP ${response.status}`);
const result = await response.json() as Record<string, unknown>;
if (result.wave !== 1
  || !Number.isInteger(result.siteCount)
  || Number(result.siteCount) < 1
  || Number(result.siteCount) > 10
  || !Array.isArray(result.sites)) {
  throw new Error("Wave 1 review response is invalid");
}
