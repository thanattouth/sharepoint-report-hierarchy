...
const response = await fetch(`https://${hostname}/api/scanner/build-candidate-manifest`, {
  method: "POST",
  headers: { "x-functions-key": functionKey },
  redirect: "manual",
});
if (response.status !== 200) throw new Error(`Candidate manifest returned HTTP ${response.status}`);
...
for (const name of [
  ...
  "preservedPilotCount",
  "siteWithoutLibrariesCount",
  "failedSiteCount",
  "waveCount",
] as const) {
  if (!Number.isInteger(result[name]) || Number(result[name]) < 0) {
    throw new Error("Candidate manifest response is invalid");
  }
}
process.stdout.write(`${JSON.stringify({
  status: "manifest-built",
  ...result,
})}\n`);
