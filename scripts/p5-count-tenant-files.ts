...
const response = await fetch(`https://${hostname}/api/scanner/count-files`, {
  method: "POST",
  headers: { "x-functions-key": functionKey },
  redirect: "manual",
});
if (response.status !== 200) throw new Error(`Tenant file count returned HTTP ${response.status}`);
...
for (const name of [
  ...
  if (!Number.isInteger(result[name]) || Number(result[name]) < 0) {
    throw new Error("Tenant file-count response is invalid");
  }
}
process.stdout.write(`${JSON.stringify({
  ...
  countedLibraryCount: result.countedLibraryCount,
  fileCount: result.fileCount,
  itemPageCount: result.itemPageCount,
  failedSiteCount: result.failedSiteCount,
  failedLibraryCount: result.failedLibraryCount,
