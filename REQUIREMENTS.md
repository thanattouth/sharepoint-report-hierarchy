...
Version: **1.6 (Manifest-Driven Customer Delivery)**
Date: **2026-07-22**
...
ต้องแสดงจำนวน Sensitive files แยกตาม SharePoint site และใช้ searchable/paginated Site Explorer
เป็น navigation หลัก หน้าแรกแสดง aggregate และ Site rows ที่ server authorize แล้ว แต่ห้ามส่ง file-level
rows จนกว่าผู้ใช้จะเลือก Site ที่อยู่ใน resolved scope อย่างชัดเจน
...
หลังเลือก Site ที่ได้รับอนุญาต ผู้ใช้ต้อง drill down ถึงรายการไฟล์ของ Site นั้นได้ โดย server ต้องอ่าน
เฉพาะ inventory partition ของ Site ที่เลือกและกรอง scope ก่อนส่งข้อมูล อย่างน้อยต้องมี:
...
### FR-009: Open SharePoint

Site detail ต้องมี `Open SharePoint` ที่เปิด Site URL ใน tab ใหม่ URL ต้องสร้างจาก cached canonical
Site hostname/path ที่ผ่าน allowlist ของ SharePoint cloud เท่านั้น และห้ามเรียก Graph แบบ real time ตอนกด
ลิงก์ Report ยังคงเป็น read-only governance metadata ตาม business scope ส่วน SharePoint เป็นผู้ authenticate
และ enforce สิทธิ์ Site/file ของผู้ใช้เอง การเห็น metadata ใน Report ไม่ได้ให้สิทธิ์เปิด Site หรือไฟล์

Direct file link ยังเป็น optional และต้องผ่าน security review แยกต่างหาก
...
File-level query ต้อง require exact authorized Site ID และอ่าน inventory partition ของ Site นั้นเท่านั้น
หน้า aggregate ห้าม enumerate file rows จากทุก Site ใน resolved scope
...
### NFR-010: Repeatable single-tenant customer delivery

- ต้อง deploy ระบบใหม่แบบ isolated ลง Entra tenant และ Azure subscription ของลูกค้าได้โดยไม่ย้าย
  pilot subscription หรือ reuse tenant-bound identity
- Tenant/subscription/resource names ต้องมาจาก validated customer manifest ที่ไม่มี secret และไม่เป็น
  production default ใน source code
- ต้องมี read-only preflight, Azure What-if, idempotent deployment, explicit admin-consent/RBAC gates,
  smoke test และ rollback plan
- App registrations, service principals, managed identities และ RBAC ต้องสร้างใหม่ใน target tenant
- SharePoint Site IDs, Entra group IDs และ sensitivity label IDs ต้อง rediscover/rebind ใน target tenant
- Business nodes สามารถ export/import หลัง validation แต่ inventory และ delta cursors ต้องสร้างใหม่จาก target scan
- ต้องเก็บ source deployment ไว้จน target UAT ผ่าน และแยก cutover/teardown เป็นคนละ approved operation
- หลัง clone repository ขั้น preflight, infrastructure, Entra, workload federation, package, publish,
  configuration bootstrap, Web deployment และ smoke check ต้องรับ customer manifest ฉบับเดียวเป็น input
- ชื่อ Function Apps, managed-identity IDs และ endpoint ที่ Azure สร้างต้อง resolve จาก deployment outputs
  ห้ามให้ operator คัดลอกไปไว้ใน manifest หรือ `.env` เพื่อเชื่อมขั้นตอน delivery
- Production delivery commands ต้องล้าง pilot env keys ก่อนเรียก child process และ fail closed เมื่อ Azure
  CLI tenant/subscription, workload federation, RBAC หรือ disabled-schedule gate ไม่ตรงกับ manifest
...
| UAT-14 | Open SharePoint | เปิด canonical Site URL ใน tab ใหม่ และ SharePoint ยัง enforce สิทธิ์ของผู้ใช้เอง |
...
- Customer UAT ผ่าน
- Customer single-tenant delivery rehearsal, rollback และ ownership handoff ผ่าน
...
- Production web hosting จะเป็น customer-owned Azure hosting หรือ managed service พร้อม data/identity boundary ใด
